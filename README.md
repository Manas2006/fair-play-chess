...
The repository runs immediately on deterministic, anonymized synthetic evidence. It also includes a real Lichess PGN → Stockfish feature path for research use.
...
The [Lichess open database](https://database.lichess.org/) publishes CC0 monthly rated-game dumps. It notes that only a subset contains evaluations, while clock comments are available from April 2017 onward. Stream a dump into inspectable game metadata:
